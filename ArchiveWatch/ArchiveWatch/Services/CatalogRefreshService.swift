...
actor CatalogRefreshService {
    ...
    // MARK: - SQLite catalog DB (Decision 017)

    /// Raw prebuilt DB hosted as a GitHub Release asset (off the Pages
    /// bandwidth budget; release CDN). URLSession follows the redirect to the
    /// asset CDN automatically.
    ...
    /// Download the full catalog DB to Caches (ETag-conditional so an
    /// unchanged DB isn't re-fetched). Returns the local path, or the cached
    /// path on 304/failure. Validated by the caller opening it via CatalogDB.
