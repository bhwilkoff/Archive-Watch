...
def clear_dead_caption_urls(items):
    """A dead subtitle leaves no published URL behind, on any platform.

    THE DEFECT (measured 2026-09-11). `audit_dead_subtitles` probes published
    tracks and, on a 404, correctly removes `subtitleHLS` so the Apple apps
    stop offering a caption that cannot load. But it only strips CAPTION
    entries whose source is None/"published" — so a `subdl` or `archive` entry
    survives, still carrying the `vttURL` of the file that just 404'd. And
    `build_web_details` emits any caption that has a vttURL.

    Result: 43 visible films advertise a subtitle menu on the web whose file
    is a 404, while the Apple apps correctly show none. That is the exact
    failure build_subtitle_assets already names in the owner's words — "a
    caption entry with no playable file gives the viewer a CC button that
    ...
    cleared = dropped = 0
    for it in items:
        if not it.get("subtitleDead") or it.get("subtitleHLS"):
            continue
        ...
        if it["captions"] is None:
            it.pop("captions", None)
    return cleared, dropped


def _is_archive_autogenerated(c):
    """archive.org's own ASR output, which it names `<file>.autogenerated.vtt`.

    NOT the same marker as Decision 043's `source == "archive-asr"`. Measured
    on the live catalog 2026-09-11: that marker matches ZERO items, while
    `.autogenerated.` matches 1,720 caption entries across 1,719 items — so
    043's predicate has been inert and the population it described is now 22%
    of every captioned film in the app (1,334 of 5,943).
    """
    u = (c.get("url") or "") + (c.get("vttURL") or "")
    return ".autogenerated." in u


def fix_autogenerated_caption_language(items):
    """An archive.org auto-transcript is English; its LABEL sometimes is not.

    Measured across every visible film carrying one (2026-09-11): 1,683 are
    labelled `en`, and 34 are labelled ar/de/it/es/ja/ru/zh/fr. All 34 were
    fetched and scored — 33 are English-dominant text (English stopword ratio
    0.15-0.22) and the 34th holds a single word. A French policier offering a
    "Deutsch" track that reads "ed / Rot / O", and an English sitcom offering
    "Italiano" that is English, are both a caption menu lying to the viewer.

    The transcripts themselves are GOOD — sampled cues read as clean,
    punctuated English ("As the red noose is drawn closer about the western
    sector of the capital...") — so the label is corrected rather than the
    track dropped. Dropping would take captions from 1,661 visible films to
    fix 34, which is a content decision, not a remediation.

    REVISIT IF archive.org ships multilingual ASR: this normalises on the
    evidence that its auto output is English in every case measured here.
    """
    fixed = 0
    for it in items:
        for c in (it.get("captions") or []):
            if not _is_archive_autogenerated(c):
                continue
            if (c.get("lang") or "en") != "en":
                c["lang"] = "en"
                c["label"] = "English"
                fixed += 1
    return fixed
...
def main():
    ...
    deadvtt, deadcaps = clear_dead_caption_urls(cat["items"])
    if deadvtt or deadcaps:
        stats["dead_caption_urls_cleared"] = deadvtt
        stats["dead_captions_dropped"] = deadcaps
    langfix = fix_autogenerated_caption_language(cat["items"])
    if langfix:
        stats["autogenerated_caption_lang_fixed"] = langfix
