...
LIMITS = {"bluesky": 300, "threads": 500, "instagram": 2200, "facebook": 5000}
...
def compose(spec: dict, platform: str) -> str:
    ...
    while len(text) > limit and body:
        longest = max(range(len(body)), key=lambda i: len(body[i]))
        cut = body[longest]
        if len(cut) <= 40:
            body.pop(longest)
        else:
            trimmed = cut[: max(40, len(cut) - (len(text) - limit) - 24)].rsplit(" ", 1)[0]
            body[longest] = trimmed.rstrip(" ,.;:—-") + "…"
            if cut.startswith('"'):
                body[longest] = body[longest].rstrip("…").rstrip() + '…"'
        text = assemble(body)
...
def post_bluesky(spec, text, card: Path, live: bool):
...
def main() -> int:
    ...
    plan = [
        ("bluesky", lambda t: post_bluesky(spec, t, card, args.live)),
