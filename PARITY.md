...
| Watch Together — chat in the program | ✅ **Twitch reads with NO credential** (anonymous `justinfan` IRC, verified against tmi.twitch.tv; 12 real messages in 20 s) — YouTube still needs the host's OAuth | ✅ same client, shared code | 🔮 | 🔮 | 🚫 | Composited INTO the broadcast so every viewer sees it, not only the host; 0.78 ms per frame |
