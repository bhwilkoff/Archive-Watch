...
| Removal requests (`takedown.html` + `shared/editorial/takedowns.json`) | ✅ | ✅ | ✅ | ✅ page, footer link | ✅ | Roku ✅. One list the pipeline honors every publish (`remediate_catalog.exclude_takedowns`, marker `excludedReason: takedown` that the rights reconcile keeps), so a removal reaches every app, search, share page and feed at once. TV-spine episodes are not catalog items: a series removal also needs the tv-rights manifest |
| "Something wrong with this film?" (pre-filled GitHub issue form) | ✅ Share sheet: Report a problem → QR | ✅ Share and more menu | ✅ Share menu | ✅ closing link on Detail | ✅ phone overflow menu · TV share overlay toggle → QR | Roku ✅ More → QR, verified on device: the code decodes to the exact form URL. Only the film id and where it was seen; nothing sent until the viewer submits |
| archive.org address or pasted link opens the film (or says it is not here) | ✅ pasted link in Search | ✅ pasted link in Search | ✅ pasted link in Search | ✅ `/details/<id>` + pasted link in search (WEB-DESIGN §4.3b) | ✅ pasted link in Search — verified on the Pixel 8a: `archive.org/details/the-scarecrow` → Buster Keaton's "The Scarecrow" | Roku 🚫 a remote keyboard is not where anyone pastes a link. Apple/Android resolve in the shared search query; `tools/test_archive_link.swift` + `ArchiveLinkTest` hold the same table as the web |
| Browse length filter (Under 60 minutes / 60 to 90 minutes / Over 90 minutes) | ✅ chip row | ✅ Filter menu picker | ✅ Filter menu picker | ✅ select (after index schema 14 publishes) | ✅ phone menu + TV chips | Roku ✅ a fifth chip (318 px step, inside the safe area) — verified on the Streaming Stick 4K: "Under 60 minutes" takes All films from 21,799 to 13,394. Same bands everywhere: `RuntimeBand` / web `LENGTHS` over index column 17 `minutes` |
...
| **Web** | 🚫 cannot broadcast | ✅ **shipped** — the invite link (`/together/#code-film`), AND since 2026-09-25 a **Join a room** entry on Library opening a code field (SHAREPLAY §11.10 had specified it; the owner looked for it and it was missing). A bare code or `#/together/<code>` joins: the room names its own film. `tools/test_web_room_join_code.mjs` |
...
| Every guest plays the HOST's copy (Decision 143) | ✅ host sends it | ✅ iOS · ✅ tvOS guests; Android phone + TV ✅; web ✅; Roku has no room surface | live Worker refuses other hosts and `..`; web guest played the host's non-default copy within 0.07 s (control: live site without it played the default); Mac guest overrode its own conflicting choice (`AWROOMCOPY`) |
