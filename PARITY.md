...
| **Watch Together — with the world** (Studio) | 🚧 player + overlays (tvOS-DESIGN §8.8): transport-menu entry, rights gate, ten-foot readout verified on the glass; Continuity Camera is the camera + mic (the mic is an `AVAudioSession` PORT, not a capture device) | 🚧 player + overlays (iOS-DESIGN §8.8): go-live sheet, layout/faders/cards sheet, health capsule, all verified on an iPhone 12 | 🚧 Phase 2 STARTED (macOS-DESIGN §B13): the Studio is the player in a production mode, `StudioSession` owns the show because §B2a makes the player the window root, health readout pinned outside the AVPlayerView HUD, camera entitlement + usage string added. Window+audio capture for a guest call is still ahead | 🔮 Phase 3 — RootEncoder is the encoder; a dependency decision of its own | 🚫 a browser cannot speak RTMP; only a server-side compositor (Phase 4) can publish from the web | One shared `StudioEngine` + our own `RTMPPublisher` (no third-party encoder — Decision 127). **Blocked on owner: the YouTube/Twitch OAuth client ids** — two strings, no secret (Decision 128); without them a stream key cannot be fetched and the sheet says so. Binding rules in `docs/WATCH-TOGETHER.md` |
