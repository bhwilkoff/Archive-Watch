...
| **Watch Together — with the world** (Studio) | 🚧 player + overlays (tvOS-DESIGN §8.8): transport-menu entry, rights gate, ten-foot readout verified on the glass; Continuity Camera is the camera + mic (the mic is an `AVAudioSession` PORT, not a capture device) | 🚧 player + overlays (iOS-DESIGN §8.8): go-live sheet, layout/faders/cards sheet, health capsule, all verified on an iPhone 12 | 🔮 Phase 2 — adds window+audio capture for a friends' call (ScreenCaptureKit) and multistream | 🔮 Phase 3 — RootEncoder is the encoder; a dependency decision of its own | 🚫 a browser cannot speak RTMP; only a server-side compositor (Phase 4) can publish from the web | One shared `StudioEngine` + our own `RTMPPublisher` (no third-party encoder — Decision 127). **Blocked on owner: the YouTube/Twitch OAuth client ids** — two strings, no secret (Decision 128); without them a stream key cannot be fetched and the sheet says so. Binding rules in `docs/WATCH-TOGETHER.md` |
| Watch Together — rights gate | ✅ refusal verified on the glass (Apple TV 4K) | ✅ both branches verified (iPhone 12) | ⏳ same predicate, surface pending | 🔮 | n/a | `StudioRights`, tier `guaranteed` = `safe_pd_age` only — **4,210 of 24,943 films** (16.9%). An unknown verdict REFUSES. Every bucket has a human sentence, guarded by `tools/test_studio_rights_coverage.py` |
