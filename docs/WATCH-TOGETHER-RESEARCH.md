# Watch Together Studio — the research it rests on (2026-09-17)

> **Renamed 2026-09-18, content kept as written.** The owner asked for the
> feature set to be reframed as **Watch Together** (with friends, via SharePlay)
> and **Watch Together Studio** (with the world), and this file was still called
> `LIVE-RIFF-RESEARCH.md` — the last place the old frame survived. The binding
> design doc is `docs/WATCH-TOGETHER.md` (Decision 127); this is the research
> underneath it.
>
> A pointer remains at the old path, because Decision 127 cites it and
> `DECISIONS.md` is append-only — a citation in an append-only file may not be
> quietly re-aimed.
>
> The prose below still uses "riffing" and "a riff show" where it describes the
> ACTIVITY — talking over a film with friends — and names RiffTrax, which is a
> real company. Those are not the product's name and were left alone: this is a
> research record, and editing a record to match a later vocabulary is how
> evidence quietly becomes advocacy.
>
