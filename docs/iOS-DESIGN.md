...
sort; iOS and macOS draw their own collection grid, which had none. The grid
carries the same `Picker("Sort")` as Browse — Popular, Top Rated, A–Z, Newest,
Oldest — in the toolbar, applied with `CatalogDB.ordered` to the loaded set.
